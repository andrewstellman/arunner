...
| `failed` | terminal heartbeat `FAILED`/`ABANDONED` reaped | no | **yes** (retryable, FR-75) |
| `auth_or_launch_failed` | claimed but no heartbeat within `launch_grace_minutes` | no | **yes** |
| `abandoned` | reclaimed terminal — a stalled slot freed by FR-74 (or a CANCEL, FR-39) | no | **yes** (stall-reclaim retryable, FR-75) |

`done` is true exactly when every run is terminal.

**Retryable terminals (FR-75).** `failed` and a FR-74 stall-reclaimed `abandoned`
are *retryable*: while a job's persisted `attempts` count is below its
`max_attempts` (default 1 = no retry), the engine requeues it (→ `queued`)
instead of leaving it terminal — so the terminal only "sticks" once the budget
is exhausted. `auth_or_launch_failed` and `completed` are never retried.
...
failed / abandoned ──[attempts < max_attempts]──▶ queued   (requeue for retry; FR-75)
                                                  (eligible after retry_backoff_seconds)
```

The `stalled ↔ running` reversibility holds **below** the reclaim threshold; the
`stalled → abandoned` edge is one-way (terminal) **unless** the retry budget
requeues it (FR-75). A stalled run whose OUTPUT is FRESH is held, never reclaimed
(the alive-but-quiet guard). The `queued → completed` done-check edge is
evaluated **before** the pool-slot dispatch, so a target already done never
claims a slot (FR-76).
...
- **Retry requeue (failed/abandoned → queued, FR-75):** at a retryable
  terminal — `failed` (a worker `FAILED`/`ABANDONED` sentinel, or a dead shell
  PID) or a FR-74 stall-reclaim (`abandoned`) — if the job's persisted
  `attempts` is below its `max_attempts` (default 1 = no retry), the engine
  **requeues** it instead of leaving it terminal: it drops the just-written
  result sentinel, restores the `queue/` claim token, rotates (clears) the
  heartbeat, and sets `retry_not_before = now + retry_backoff_seconds` so the
  retry is dispatch-eligible only after the backoff (on the `ARUNNER_NOW`
  clock). The attempt count is incremented at each dispatch, so the requeue is
  bounded — once exhausted the terminal stands (`failed` stays `failed`; a
  stall-reclaim stays `abandoned`, FR-74's honest "gave up waiting"). A retry
  is **resume-not-restart**: it keeps the worker's OUTPUT and re-derives the
  FR-76 `done_check` (a now-satisfied target is skipped — `SKIPPED:done` — with
  no wasted attempt). **`auth_or_launch_failed` is never retried** (the
  transient-vs-fatal default). FR-6 holds: a requeued job is `queued`, claimed
  once per attempt — never double-dispatched. `max_attempts` supersedes FR-74's
  reserved `stall_retries`. Single-prompt jobs only (multistep retry is a
