...
| UC-2 (alt: read-only monitor sidecar) | opens `arunner monitor <run-dir>` in a second terminal; confirms it renders the live table from disk and refreshes ACTIVITY/HB-AGE while the orchestrator is blocked — and writes nothing (no `.tick.lock`, no control file, no state mutation) (FR-59) | sidecar (rung-independent) | `test_monitor` (never-writes pin; live-heartbeat vs per-tick-state freshness; shared-renderer-no-fork) | per-OS | **VERIFIED** (engine/CLI leg; per-OS recorded run = operator) |
| UC-2 (alt: interactive read-only TUI) | opens `arunner tui`; browses run-dirs newest-first, opens one to the live table (same `_format_table` as `monitor`, no fork), drills into one entry's full record + heartbeat history + `results/`, tails its `heartbeat.ndjson`/the run `journal.ndjson` — strictly read-only; Textual gated behind the `[tui]` extra, engine path imports none of it (FR-62) | sidecar (rung-independent, optional) | `test_tui` (never-writes pin [run-dir + runs-root snapshot]; run-view-reuses-renderer-no-fork; engine-path-has-no-Textual-import + clean degradation) | per-OS | **VERIFIED** (engine/CLI/data leg; per-OS recorded TUI run = operator) |
